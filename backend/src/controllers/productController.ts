...
const sugarSchema = z.object({
  value: z.union([z.literal(0), z.literal(25), z.literal(50), z.literal(100)]),
  label: z.string()
});

const iceSchema = z.object({
  value: z.enum(['none', 'normal', 'extra']),
  label: z.string()
});
...
const productSchema = z.object({
  ...
  price: z.number().positive('السعر الأساسي يجب أن يكون موجب'),
  isFeatured: z.union([z.boolean(), z.number()]).optional(),
  isSpecial: z.union([z.boolean(), z.number()]).optional(),
  availableSizes: z.array(sizeSchema).default([]),
  availableSugar: z.array(sugarSchema).default([]),
  availableIce: z.array(iceSchema).default([]),
