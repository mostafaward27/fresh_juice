    ...
    <div className="py-10 bg-slate-50/50 flex-grow text-right">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {cart.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="lg:col-span-2 flex flex-col gap-4">
              {cart.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 bg-white border border-slate-100 rounded-[28px] shadow-sm gap-4"
                >
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        ...
                        {item.selectedExtras.map(extra => {
                          ...
                          return (
                            <span key={extra} className="text-[10px] font-bold bg-orange-50 text-brand-orange-600 px-2 py-0.5 rounded-md">
                              +{extraOpt?.label}
                            </span>
                          );
                        })}
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0 border-slate-50">
                    ...
                    {/* Quantity controls */}
                    <div className="flex items-center gap-3.5 bg-slate-50 border border-slate-100 p-1.5 rounded-xl select-none">
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity - 1)}
                        className="p-1.5 bg-white hover:bg-slate-100 border border-slate-200 rounded-lg transition-colors"
                      >
                        <Minus className="w-3.5 h-3.5 text-slate-600" />
                      </button>
                      <span className="text-sm font-black text-slate-800 w-6 text-center number-font">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-left min-w-[70px]">
                        ...
                        <div className="flex items-baseline justify-end gap-0.5">
                          <span className="text-base font-black text-slate-800 number-font">
                      ...
                      <button
                        ...
                        className="p-3 text-slate-400 hover:text-rose-500 bg-slate-50 hover:bg-rose-50 rounded-xl transition-all"
                        title="حذف من السلة"
                      >
            ...
            <div className="bg-white border border-slate-100 rounded-[32px] p-6 shadow-sm flex flex-col gap-6 sticky top-24">
              <h3 className="font-extrabold text-slate-800 text-lg border-b border-slate-50 pb-4">إجمالي الطلب</h3>
          ...
          /* Empty Cart State */
          <div className="text-center py-24 bg-white border border-slate-100 rounded-[40px] shadow-sm max-w-xl mx-auto p-8 flex flex-col items-center">
            <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center text-4xl mb-6 animate-pulse-slow">
              🛒
            </div>
            <h3 className="text-xl font-bold text-slate-800 mb-2">سلتك فارغة تماماً</h3>
            ...
            <Link to="/menu">
              <Button size="lg">شاهد منيو العصائر</Button>
            </Link>
          </div>
        )}
